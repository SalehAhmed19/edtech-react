...
const CartsTable = ({ headers, data, className = "" }) => {
  ...
  return (
    <div
      className={`overflow-x-auto bg-white rounded-lg shadow-md ${className}`}
    >
      <table className="min-w-full divide-y divide-dashed divide-gray-300">
          ...
          <tr>
            {headers.map((header, index) => (
              <th
                key={index}
                scope="col"
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-dashed border-gray-300"
              >
                {header}
              </th>
            ))}
        ...
        <tbody className="divide-y divide-dashed divide-gray-200">
          {data.map((row, rowIndex) => (
            <tr key={rowIndex}>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 border-b border-dashed border-gray-200">
